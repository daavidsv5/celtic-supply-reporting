 ...
 *
 * Requires env var: SHOPTET_API_TOKEN_NL
 * Or set it directly in .env.local as SHOPTET_API_TOKEN_NL=...
 */
...
const API_TOKEN      = process.env.SHOPTET_API_TOKEN_NL;
const API_BASE       = 'https://api.myshoptet.com/api';
const DATA_DIR       = path.join(__dirname, '..', 'data');
const CACHE_FILE     = path.join(__dirname, 'orders_cache_nl.json');
const CATEGORY_MAP_CACHE = path.join(__dirname, 'category_map_nl.json');
const LOG_FILE       = path.join(__dirname, 'fetchShoptetDataNL.log');
...
if (!API_TOKEN) {
  console.error('ERROR: SHOPTET_API_TOKEN_NL env variable is not set.');
...
function processOrders(orders, productCategoryMap = {}) {
  ...
  for (const order of orders) {
    ...
    // Shipping & payment
    const shippingName = order.shipping?.name || 'Onbekend';
    const paymentName  = order.paymentMethod?.name || 'Onbekend';
...
export const realDataNL: RealDailyRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'realDataNL.ts'), content, 'utf8');
  log(`realDataNL.ts: ${records.length} days`);
}
...
export const productDataNL: ProductSaleRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'productDataNL.ts'), content, 'utf8');
  log(`productDataNL.ts: ${records.length} records`);
}
...
export const marginDataNL: MarginDailyRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'marginDataNL.ts'), content, 'utf8');
  log(`marginDataNL.ts: ${records.length} days`);
}
...
export const hourlyDataNL: HourlyPoint[] = ${JSON.stringify(hourlyData, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'hourlyDataNL.ts'), content, 'utf8');
  log(`hourlyDataNL.ts: ${hourlyData.length} cells`);
}
...
export const crossSellDataNL: CrossSellData = ${JSON.stringify(crossSell, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'crossSellDataNL.ts'), content, 'utf8');
  log(`crossSellDataNL.ts: ${crossSell.pairs.length} pairs`);
}
...
export const retentionDataNL: { dates: string[]; revenues: number[]; revsVat: number[] }[] = ${JSON.stringify(retention, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'retentionDataNL.ts'), content, 'utf8');
  log(`retentionDataNL.ts: ${retention.length} customers`);
}
...
export const orderValueDataNL: OrderValueRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'orderValueDataNL.ts'), content, 'utf8');
  log(`orderValueDataNL.ts: ${records.length} orders`);
}
...
export const categoryDataNL: CategoryRevenueRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'categoryDataNL.ts'), content, 'utf8');
  log(`categoryDataNL.ts: ${records.length} records`);
}
...
export const brandDataNL: BrandRevenueRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'brandDataNL.ts'), content, 'utf8');
  log(`brandDataNL.ts: ${records.length} records`);
}
...
export const shippingPaymentDataNL: ShippingPaymentRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'shippingPaymentDataNL.ts'), content, 'utf8');
  log(`shippingPaymentDataNL.ts: ${records.length} records`);
}
...
// ── Main ───────────────────────────────────────────────────────────────────────
async function main() {
  log(`=== fetchShoptetDataNL.js START (${FULL_SYNC ? 'FULL SYNC' : 'INCREMENTAL'}) ===`);
  ...
  log('=== fetchShoptetDataNL.js DONE ===');
}
