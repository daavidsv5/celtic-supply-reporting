/**
 * fetchShoptetDataSK.js
 * Fetches orders from Shoptet Private API (SK) and generates data/*.ts files.
 *
 * Run:  node scripts/fetchShoptetDataSK.js           # incremental (last 7 days)
 *       node scripts/fetchShoptetDataSK.js --full    # full sync (all orders from 2023-01-01)
 *
 * Requires env var: SHOPTET_API_TOKEN_SK
 * Or set it directly in .env.local as SHOPTET_API_TOKEN_SK=...
 */
...
const API_TOKEN      = process.env.SHOPTET_API_TOKEN_SK;
const API_BASE       = 'https://api.myshoptet.com/api';
const DATA_DIR       = path.join(__dirname, '..', 'data');
const CACHE_FILE     = path.join(__dirname, 'orders_cache_sk.json');
const CATEGORY_MAP_CACHE = path.join(__dirname, 'category_map_sk.json');
const LOG_FILE       = path.join(__dirname, 'fetchShoptetDataSK.log');

const FULL_SYNC        = process.argv.includes('--full');
const REPROCESS_ONLY   = process.argv.includes('--reprocess'); // skip API, just reprocess existing cache
const INCREMENTAL_DAYS = 10;   // days to look back for incremental sync
const BATCH_SIZE       = 3;    // concurrent detail requests (SK API rate limit is strict)
...
// Status IDs that mean cancelled (Stornovaná)
const CANCELLED_STATUS_IDS = new Set([-4]);

// SK launch date — orders before this date are test orders, exclude from data
const SK_LAUNCH_DATE = '2024-06-01';

// Allegro marketplace — exclude from SK retail report (wholesale, distorts metrics)
function isAllegroOrder(order) {
  const email = (order.email || '').toLowerCase();
  return email.includes('allegromail') || email.includes('allegro@celtic-supply');
}

if (!API_TOKEN) {
  console.error('ERROR: SHOPTET_API_TOKEN_SK env variable is not set.');
...
// We take the date portion directly — it's already in local (SK) time.
...
function processOrders(orders, productCategoryMap = {}) {
  ...
  for (const order of orders) {
    ...
    // Skip orders before SK launch date (test orders)
    if (date < SK_LAUNCH_DATE) continue;

    // Skip Allegro marketplace orders (wholesale, excluded from retail report)
    ...
    for (const item of productItems) {
      ...
      const catEntry = (item.productGuid && productCategoryMap[item.productGuid])
        ...
        : { root: 'Nezaradené', sub: '' };
      const rootCat = catEntry.root || 'Nezaradené';
      ...
      // Brands
      const brand = ((item.productGuid && productCategoryMap[item.productGuid]?.brand) || '').trim() || 'Nezaradené';
      const bk = `${date}|${brand}`;
    ...
    // Shipping & payment
    const shippingName = order.shipping?.name || 'Neznáme';
    const paymentName  = order.paymentMethod?.name || 'Neznáme';
...
function writeRealData(byDay) {
  ...
  const content = `// Auto-generated by scripts/fetchShoptetDataSK.js — last update: ${now}
...
export const realDataSK: RealDailyRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'realDataSK.ts'), content, 'utf8');
  log(`realDataSK.ts: ${records.length} days`);
}

function writeProductData(products) {
  const records = sortByDate(products);
  const content = `// Auto-generated by scripts/fetchShoptetDataSK.js — last update: ${now}
// SK: product sales (cancelled excluded)
...
export const productDataSK: ProductSaleRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'productDataSK.ts'), content, 'utf8');
  log(`productDataSK.ts: ${records.length} records`);
}
...
export const marginDataSK: MarginDailyRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'marginDataSK.ts'), content, 'utf8');
  log(`marginDataSK.ts: ${records.length} days`);
}
...
export const hourlyDataSK: HourlyPoint[] = ${JSON.stringify(hourlyData, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'hourlyDataSK.ts'), content, 'utf8');
  log(`hourlyDataSK.ts: ${hourlyData.length} cells`);
}
...
export const crossSellDataSK: CrossSellData = ${JSON.stringify(crossSell, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'crossSellDataSK.ts'), content, 'utf8');
  log(`crossSellDataSK.ts: ${crossSell.pairs.length} pairs`);
}
...
export const retentionDataSK: { dates: string[]; revenues: number[]; revsVat: number[] }[] = ${JSON.stringify(retention, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'retentionDataSK.ts'), content, 'utf8');
  log(`retentionDataSK.ts: ${retention.length} customers`);
}
...
export const orderValueDataSK: OrderValueRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'orderValueDataSK.ts'), content, 'utf8');
  log(`orderValueDataSK.ts: ${records.length} orders`);
}

function writeCategoryData(categoryData) {
  const records = sortByDate(categoryData);
  const content = `// Auto-generated by scripts/fetchShoptetDataSK.js — last update: ${now}
// SK: daily revenue by category (EUR, cancelled excluded)
...
export const categoryDataSK: CategoryRevenueRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'categoryDataSK.ts'), content, 'utf8');
  log(`categoryDataSK.ts: ${records.length} records`);
}

function writeBrandData(brandData) {
  const records = sortByDate(brandData);
  const content = `// Auto-generated by scripts/fetchShoptetDataSK.js — last update: ${now}
// SK: daily revenue by brand/manufacturer (EUR, cancelled excluded)
...
export const brandDataSK: BrandRevenueRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'brandDataSK.ts'), content, 'utf8');
  log(`brandDataSK.ts: ${records.length} records`);
}
...
export const shippingPaymentDataSK: ShippingPaymentRecord[] = ${JSON.stringify(records, null, 2)};
`;
  fs.writeFileSync(path.join(DATA_DIR, 'shippingPaymentDataSK.ts'), content, 'utf8');
  log(`shippingPaymentDataSK.ts: ${records.length} records`);
}
...
// ── Main ───────────────────────────────────────────────────────────────────────
async function main() {
  const modeLabel = REPROCESS_ONLY ? 'REPROCESS ONLY' : FULL_SYNC ? 'FULL SYNC' : 'INCREMENTAL';
  log(`=== fetchShoptetDataSK.js START (${modeLabel}) ===`);

  let cache;

  if (REPROCESS_ONLY) {
    // Skip all API calls — just load existing cache and reprocess
    if (!fs.existsSync(CACHE_FILE)) {
      log('ERROR: No cache file found. Run --full first to populate the cache.');
      process.exit(1);
    }
    cache = loadCache();
    log(`Loaded ${Object.keys(cache).length} orders from cache. Skipping API fetch.`);
  } else {
  ...
  log('=== fetchShoptetDataSK.js DONE ===');
}
